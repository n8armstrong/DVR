...
open class Session: URLSession {
    ...
    func finishTask(_ task: URLSessionTask, interaction: Interaction, playback: Bool) {
        ...
        if let delegate = delegate as? URLSessionTaskDelegate {
            let bytes = Int64(interaction.responseData?.count ?? 0)
    ...
    fileprivate func addDataTask(_ request: URLRequest, completionHandler: ((Data?, Foundation.URLResponse?, NSError?) -> Void)? = nil) -> URLSessionDataTask {
        var request = request
        let modifiedRequest = backingSession.configuration.httpAdditionalHeaders.map { request.requestByAppendingHeaders($0) } ?? request
    ...
    fileprivate func addUploadTask(_ request: URLRequest, fromData data: Data?, completionHandler: SessionUploadTask.Completion? = nil) -> URLSessionUploadTask {
        var request = request
        var modifiedRequest = backingSession.configuration.httpAdditionalHeaders.map { request.requestByAppendingHeaders($0) } ?? request
        modifiedRequest = data.map { modifiedRequest.requestWithBody($0) } ?? modifiedRequest
